export type Lang = 'zh' | 'en'

const zh = {
  // App
  appName: '口袋记账',
  appVersion: '口袋记账 v0.2.0',

  // Common
  save: '保存',
  saving: '保存中…',
  cancel: '取消',
  delete: '删除',
  edit: '编辑',
  set: '设置',
  expense: '支出',
  income: '收入',
  transfer: '转账',
  uncategorized: '未分类',
  balance: '结余',

  // Date format: t('yearMonthFmt', { year, month })
  yearMonthFmt: '{year}年{month}月',

  // Weekday labels (home formatDateLabel)
  weekday0: '周日', weekday1: '周一', weekday2: '周二', weekday3: '周三',
  weekday4: '周四', weekday5: '周五', weekday6: '周六',

  // Calendar grid headers
  calDay0: '日', calDay1: '一', calDay2: '二', calDay3: '三',
  calDay4: '四', calDay5: '五', calDay6: '六',

  // BottomNav
  navHome: '首页',
  navCalendar: '日历',
  navStats: '统计',
  navSettings: '设置',

  // Home
  monthlyExpense: '本月支出',
  monthlyBudget: '月度预算',
  overBudget: '⚠ 超出预算',
  budgetPctUsed: '{pct}% 已使用',
  billDetail: '账单明细',
  txCountSuffix: '笔',
  searchPlaceholder: '搜索备注关键词...',
  noRecords: '本月暂无记录',
  noRecordsHint: '点击底部 + 开始记账吧',
  noFilterResults: '没有符合条件的记录',
  clearFilter: '清除筛选',
  today: '今天',
  yesterday: '昨天',
  filterDateRange: '日期范围',
  filterTo: '至',
  filterType: '类型',
  filterCategory: '分类',
  filterTitle: '筛选',
  filterReset: '重置',
  filterConfirm: '确认筛选',

  // Calendar
  noRecordsForDay: '当天没有记录',

  // AddTransaction
  addTx: '记一笔',
  editTx: '编辑记录',
  notePlaceholder: '添加备注（选填）',
  dayBeforeYesterday: '前天',
  other: '其他',
  amountRequired: '请输入金额',
  categoryRequired: '请选择分类',
  saveFailed: '保存失败：',
  savedContinue: '已保存，继续记账 ✓',
  addAgain: '再记',
  update: '更新',

  // TransactionSheet
  txDetail: '交易详情',
  typeLabel: '类型',
  currencyLabel: '货币',
  dateLabel: '日期',
  noteLabel: '备注',
  confirmDelete: '确认删除',
  confirmDeleteMsg: '此操作无法撤销，确定要删除这条记录吗？',
  deleting: '删除中…',
  originalLabel: '原始：',

  // Charts / Stats
  statsTitle: '统计',
  tabOverview: '月度趋势',
  tabDaily: '每日支出',
  tabCategory: '分类占比',
  last6Months: '近6个月收支',
  noExpenseData: '本月暂无支出记录',
  noIncomeData: '本月暂无收入记录',

  // Budget
  budgetTitle: '预算管理',
  monthlyTotalBudget: '月度总预算',
  budgetLimit: '上限',
  overLimit: '已超出！',
  noBudgetSet: '尚未设置总预算',
  pctUsed: '{pct}% 已使用',
  over: '超出',
  remaining: '剩余',
  categoryBudget: '分类预算',
  noCategoryBudget: '还没有分类预算',
  addCategoryBudgetHint: '在下方点击分类来添加预算',
  addCategoryBudgetSection: '添加分类预算',
  monthlyLimitLabel: '月度限额（{currency}）',

  // Settings
  settingsTitle: '设置',
  personalAccount: '个人账户',
  currencySection: '货币',
  preferredCurrency: '偏好货币',
  preferredCurrencyDesc: '用于统计和首页展示',
  defaultCurrency: '默认记账货币',
  defaultCurrencyDesc: '新增交易时默认选中',
  selectPreferredCurrency: '选择偏好货币',
  selectDefaultCurrency: '选择默认记账货币',
  useLastUsedCurrency: '上次使用的货币',
  useLastUsedCurrencyDesc: '记一笔时自动跟上次保存的币种',

  // Category sort
  categorySortLabel: '类目排序',
  categorySortDesc: '记一笔时类目的显示顺序',
  categorySortModeTitle: '类目排序方式',
  categorySortModeCustom: '自定义顺序',
  categorySortModeCustomDesc: '在管理页里逐项调整位置',
  categorySortModeRecent: '最近使用优先',
  categorySortModeRecentDesc: '最近用过的类目自动靠前',
  categorySortManageOrder: '管理类目顺序',
  categoryOrderTitle: '类目顺序',
  categoryOrderEmpty: '当前还没有该类型的类目',
  categoryOrderUp: '上移',
  categoryOrderDown: '下移',
  otherSection: '其他',
  budgetManagement: '预算管理',
  dataExport: '数据导出',
  comingSoon: '即将推出',
  signOut: '退出登录',
  languageSection: '语言',
  languageLabel: '语言',
  languageDesc: '切换界面语言',
  selectLanguage: '选择语言',
  langZh: '中文',
  langEn: 'English',

  // Daily reminder
  reminderSection: '提醒',
  reminderLabel: '每日记账提醒',
  reminderDesc: '每天定时提醒你记账',
  reminderTitle: '每日记账提醒',
  reminderEnable: '启用提醒',
  reminderTime: '提醒时间',
  reminderNotifyTitle: '记账提醒',
  reminderNotifyBody: '今天的账记了吗？',
  reminderWebHint: '提醒只在安装的 Android APP 里生效，浏览器版无效。',
  reminderPermissionDenied: '没有通知权限。请去系统设置里手动开启。',
  reminderOff: '未开启',

  // Auth
  emailLabel: '邮箱',
  emailPlaceholder: '请输入邮箱地址',
  passwordLabel: '密码',
  passwordPlaceholder: '请输入密码（至少 6 位）',
  passwordTooShort: '密码至少需要 6 位',
  confirmPasswordLabel: '确认密码',
  confirmPasswordPlaceholder: '再次输入密码',
  passwordMismatch: '两次输入的密码不一致',
  loginError: '邮箱或密码错误，请重试',
  emailExistsError: '该邮箱已注册，请直接登录',
  registerSuccess: '注册成功！请检查邮箱，点击验证链接后即可登录。',
  loadingBtn: '请稍候...',
  loginBtn: '登 录',
  registerBtn: '注 册',
  loginTab: '登录',
  registerTab: '注册',
  noAccount: '还没有账号？',
  registerNow: '立即注册',
  loginAgreement: '登录即表示你同意我们的服务条款和隐私政策',

  // Forgot password (Auth.tsx forgot mode)
  forgotPasswordLink: '忘记密码？',
  forgotPasswordTitle: '重置密码',
  forgotPasswordDesc: '输入注册邮箱，我们会发送重置链接',
  forgotSendBtn: '发送重置邮件',
  forgotSending: '发送中...',
  forgotEmailSent: '重置邮件已发送，请检查邮箱',
  forgotBackToLogin: '返回登录',
  forgotRateLimit: '请求过于频繁，请稍后再试',
  forgotSendFailed: '发送失败，请稍后再试',

  // About page
  aboutPageTitle: '关于本应用',
  backAriaLabel: '返回',
  loginAgreementLinkText: '服务条款和隐私政策',

  // Reset password landing page (ResetPassword.tsx)
  resetPasswordTitle: '设置新密码',
  resetPasswordDesc: '请输入你的新密码',
  resetNewPasswordLabel: '新密码',
  resetNewPasswordPlaceholder: '至少 6 位',
  resetConfirmPasswordLabel: '确认新密码',
  resetConfirmPasswordPlaceholder: '再次输入',
  resetSaveBtn: '保存新密码',
  resetSaving: '保存中...',
  resetSuccess: '密码已更新',
  resetLinkExpired: '重置链接已过期，请重新申请',
  resetSaveFailed: '保存失败，请重试',
  resetGoToSignIn: '前往登录页重新申请',

  // Change Password (Settings page)
  changePwdSection: '账号安全',
  changePwdLabel: '修改密码',
  changePwdTitle: '修改密码',
  changePwdCurrentLabel: '当前密码',
  changePwdCurrentPlaceholder: '输入当前密码',
  changePwdNewLabel: '新密码',
  changePwdNewPlaceholder: '至少 6 位',
  changePwdConfirmLabel: '确认新密码',
  changePwdConfirmPlaceholder: '再次输入',
  changePwdSameAsOld: '新密码不能与当前密码相同',
  changePwdSaveBtn: '保存新密码',
  changePwdSaving: '保存中...',
  changePwdWrongCurrent: '当前密码错误',
  changePwdUpdateFailed: '修改失败，请重试',
  changePwdSuccess: '密码已修改，即将退出登录',

  // Export (Settings page)
  exportRangeAll: '全部历史',
  exportRangeThisMonth: '本月',
  exportRangeCustom: '自定义范围',
  exportFromLabel: '从',
  exportToLabel: '至',
  exportBtn: '导出 CSV',
  exporting: '导出中...',
  exportSuccess: '已导出 {count} 条记录',
  exportNoData: '所选时间段内无记录',
  exportFailed: '导出失败',
}

const en: typeof zh = {
  appName: 'Pocket Ledger',
  appVersion: 'Pocket Ledger v0.2.0',

  save: 'Save',
  saving: 'Saving…',
  cancel: 'Cancel',
  delete: 'Delete',
  edit: 'Edit',
  set: 'Set',
  expense: 'Expense',
  income: 'Income',
  transfer: 'Transfer',
  uncategorized: 'Uncategorized',
  balance: 'Balance',

  yearMonthFmt: '{month}/{year}',

  weekday0: 'Sun', weekday1: 'Mon', weekday2: 'Tue', weekday3: 'Wed',
  weekday4: 'Thu', weekday5: 'Fri', weekday6: 'Sat',

  calDay0: 'Su', calDay1: 'Mo', calDay2: 'Tu', calDay3: 'We',
  calDay4: 'Th', calDay5: 'Fr', calDay6: 'Sa',

  navHome: 'Home',
  navCalendar: 'Calendar',
  navStats: 'Stats',
  navSettings: 'Settings',

  monthlyExpense: 'Monthly Expense',
  monthlyBudget: 'Budget',
  overBudget: '⚠ Over Budget',
  budgetPctUsed: '{pct}% used',
  billDetail: 'Transactions',
  txCountSuffix: '',
  searchPlaceholder: 'Search notes...',
  noRecords: 'No records this month',
  noRecordsHint: 'Tap + to start tracking',
  noFilterResults: 'No matching records',
  clearFilter: 'Clear filters',
  today: 'Today',
  yesterday: 'Yesterday',
  filterDateRange: 'Date Range',
  filterTo: 'to',
  filterType: 'Type',
  filterCategory: 'Category',
  filterTitle: 'Filter',
  filterReset: 'Reset',
  filterConfirm: 'Apply',

  noRecordsForDay: 'No records for this day',

  addTx: 'Add',
  editTx: 'Edit Record',
  notePlaceholder: 'Add note (optional)',
  dayBeforeYesterday: '2 days ago',
  other: 'Other',
  amountRequired: 'Please enter amount',
  categoryRequired: 'Please select category',
  saveFailed: 'Save failed: ',
  savedContinue: 'Saved ✓',
  addAgain: 'More',
  update: 'Update',

  txDetail: 'Transaction',
  typeLabel: 'Type',
  currencyLabel: 'Currency',
  dateLabel: 'Date',
  noteLabel: 'Note',
  confirmDelete: 'Confirm Delete',
  confirmDeleteMsg: 'This cannot be undone. Delete this record?',
  deleting: 'Deleting…',
  originalLabel: 'Original: ',

  statsTitle: 'Stats',
  tabOverview: 'Trend',
  tabDaily: 'Daily',
  tabCategory: 'Category',
  last6Months: 'Last 6 Months',
  noExpenseData: 'No expenses this month',
  noIncomeData: 'No income this month',

  budgetTitle: 'Budget',
  monthlyTotalBudget: 'Monthly Budget',
  budgetLimit: 'Limit',
  overLimit: 'Over limit!',
  noBudgetSet: 'No budget set',
  pctUsed: '{pct}% used',
  over: 'Over',
  remaining: 'Left',
  categoryBudget: 'By Category',
  noCategoryBudget: 'No category budgets yet',
  addCategoryBudgetHint: 'Tap a category below to add a budget',
  addCategoryBudgetSection: 'Add Category Budget',
  monthlyLimitLabel: 'Monthly limit ({currency})',

  settingsTitle: 'Settings',
  personalAccount: 'Personal Account',
  currencySection: 'Currency',
  preferredCurrency: 'Preferred Currency',
  preferredCurrencyDesc: 'Used for stats and home',
  defaultCurrency: 'Default Currency',
  defaultCurrencyDesc: 'Pre-selected when adding',
  selectPreferredCurrency: 'Select Preferred Currency',
  selectDefaultCurrency: 'Select Default Currency',
  useLastUsedCurrency: 'Last used currency',
  useLastUsedCurrencyDesc: 'Auto-follow the currency you saved last',

  // Category sort
  categorySortLabel: 'Category order',
  categorySortDesc: 'How categories are sorted on the add page',
  categorySortModeTitle: 'Category sort',
  categorySortModeCustom: 'Custom order',
  categorySortModeCustomDesc: 'Arrange each item on the manage page',
  categorySortModeRecent: 'Most recent first',
  categorySortModeRecentDesc: 'Recently used categories float to the top',
  categorySortManageOrder: 'Manage category order',
  categoryOrderTitle: 'Category order',
  categoryOrderEmpty: 'No categories of this type yet',
  categoryOrderUp: 'Move up',
  categoryOrderDown: 'Move down',
  otherSection: 'Other',
  budgetManagement: 'Budget',
  dataExport: 'Export Data',
  comingSoon: 'Coming Soon',
  signOut: 'Sign Out',
  languageSection: 'Language',
  reminderSection: 'Reminders',
  reminderLabel: 'Daily Logging Reminder',
  reminderDesc: 'Remind me to log expenses at a chosen time',
  reminderTitle: 'Daily Reminder',
  reminderEnable: 'Enable',
  reminderTime: 'Reminder Time',
  reminderNotifyTitle: 'Logging Reminder',
  reminderNotifyBody: 'Have you logged today\'s expenses?',
  reminderWebHint: 'Reminders only work in the installed Android app, not in the browser.',
  reminderPermissionDenied: 'Notification permission denied. Please enable it in system settings.',
  reminderOff: 'Off',
  languageLabel: 'Language',
  languageDesc: 'Switch interface language',
  selectLanguage: 'Select Language',
  langZh: '中文',
  langEn: 'English',

  emailLabel: 'Email',
  emailPlaceholder: 'Enter your email',
  passwordLabel: 'Password',
  passwordPlaceholder: 'Password (min 6 chars)',
  passwordTooShort: 'Password must be at least 6 characters',
  confirmPasswordLabel: 'Confirm Password',
  confirmPasswordPlaceholder: 'Re-enter password',
  passwordMismatch: 'Passwords do not match',
  loginError: 'Incorrect email or password, please try again',
  emailExistsError: 'Email already registered, please sign in',
  registerSuccess: 'Registered! Check your email and click the verification link to sign in.',
  loadingBtn: 'Please wait...',
  loginBtn: 'Sign In',
  registerBtn: 'Sign Up',
  loginTab: 'Sign In',
  registerTab: 'Sign Up',
  noAccount: "Don't have an account?",
  registerNow: 'Sign Up',
  loginAgreement: 'By signing in you agree to our Terms of Service and Privacy Policy',

  // Forgot password (Auth.tsx forgot mode)
  forgotPasswordLink: 'Forgot password?',
  forgotPasswordTitle: 'Reset Password',
  forgotPasswordDesc: 'Enter your email to receive a reset link',
  forgotSendBtn: 'Send Reset Email',
  forgotSending: 'Sending...',
  forgotEmailSent: 'Reset email sent, please check your inbox',
  forgotBackToLogin: 'Back to Sign In',
  forgotRateLimit: 'Too many requests, please try again later',
  forgotSendFailed: 'Failed to send, please try again',

  aboutPageTitle: 'About',
  backAriaLabel: 'Back',
  loginAgreementLinkText: 'Terms of Service and Privacy Policy',

  // Reset password landing page (ResetPassword.tsx)
  resetPasswordTitle: 'Set New Password',
  resetPasswordDesc: 'Please enter your new password',
  resetNewPasswordLabel: 'New Password',
  resetNewPasswordPlaceholder: 'At least 6 characters',
  resetConfirmPasswordLabel: 'Confirm New Password',
  resetConfirmPasswordPlaceholder: 'Re-enter',
  resetSaveBtn: 'Save New Password',
  resetSaving: 'Saving...',
  resetSuccess: 'Password updated',
  resetLinkExpired: 'Reset link expired, please request again',
  resetSaveFailed: 'Failed to save, please try again',
  resetGoToSignIn: 'Go to sign in page',

  // Change Password (Settings page)
  changePwdSection: 'Account Security',
  changePwdLabel: 'Change Password',
  changePwdTitle: 'Change Password',
  changePwdCurrentLabel: 'Current Password',
  changePwdCurrentPlaceholder: 'Enter current password',
  changePwdNewLabel: 'New Password',
  changePwdNewPlaceholder: 'At least 6 characters',
  changePwdConfirmLabel: 'Confirm New Password',
  changePwdConfirmPlaceholder: 'Re-enter',
  changePwdSameAsOld: 'New password cannot be the same as current',
  changePwdSaveBtn: 'Save New Password',
  changePwdSaving: 'Saving...',
  changePwdWrongCurrent: 'Current password is incorrect',
  changePwdUpdateFailed: 'Failed to update, please try again',
  changePwdSuccess: 'Password changed, signing out...',

  // Export (Settings page)
  exportRangeAll: 'All History',
  exportRangeThisMonth: 'This Month',
  exportRangeCustom: 'Custom Range',
  exportFromLabel: 'From',
  exportToLabel: 'To',
  exportBtn: 'Export CSV',
  exporting: 'Exporting...',
  exportSuccess: 'Exported {count} records',
  exportNoData: 'No records in selected range',
  exportFailed: 'Export failed',
}

export const translations = { zh, en }
export type TranslationKey = keyof typeof zh
