...
export default function AddTransaction() {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="border-t border-gray-100 bg-white px-4 pt-3 pb-3">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {['7','8','9'].map(k => (
            <button key={k} onClick={() => numPress(k)}
              className="py-[14px] bg-gray-50 rounded-xl text-lg font-medium text-gray-800 active:scale-95 transition-transform">
              {k}
            </button>
          ))}
          <button onClick={numDel}
            className="py-[14px] bg-white border border-gray-100 rounded-xl text-base text-gray-500 active:scale-95 transition-transform">
            ⌫
          </button>
          ...
          {editTx ? (
            <div className="py-[14px] bg-gray-50 rounded-xl" />
          ) : (
            <button onClick={() => handleSave(true)} disabled={submitting}
              className="py-[14px] bg-blue-500 text-white rounded-xl text-sm font-semibold active:scale-95 transition-transform disabled:opacity-50">
              再记
            </button>
          )}
          ...
          <button onClick={() => numPress('.')}
            className="py-[14px] bg-white border border-gray-100 rounded-xl text-lg text-gray-600 active:scale-95 transition-transform">
            .
          </button>
        </div>
      </div>
