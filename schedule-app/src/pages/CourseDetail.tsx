...
export default function CourseDetail() {
  ...
  if (!course) {
    return (
      <Layout title={t('courseDetail.notFoundTitle')} hideNav showBack onBack={() => navigate('/courses')} syncKeys={SYNC_KEYS}>
        <div className="p-8 text-center text-dim">{t('courseDetail.notFound')}</div>
      </Layout>
    )
  }
    ...
    >
      <div className="p-4 space-y-5">
        <section className="p-4 rounded-xl bg-card border border-border">
          ...
          {course.credit && (
            <div className="text-xs text-dim">{t('courseDetail.credit', { n: course.credit })}</div>
          )}
          {course.lecturer && (
        ...
        {weightRows.length > 0 && (
          <section>
            <h3 className="text-xs font-semibold tracking-wider text-muted uppercase mb-2">
              {t('courseDetail.assessmentWeightHeading', { pct: totalWeight.toFixed(0) })}
            </h3>
        ...
        {courseEvents.length === 0 ? (
          ...
          <>
            {datedEvents.length > 0 && (
              <section>
                <h3 className="text-xs font-semibold tracking-wider text-muted uppercase mb-2">
                  {t('courseDetail.eventsHeading', { n: datedEvents.length })}
                </h3>
            ...
            {tbdEvents.length > 0 && (
              <section>
                <h3 className="text-xs font-semibold tracking-wider text-emerald-500 mb-2">
                  {t('timeline.tbdHeading', { n: tbdEvents.length })}
                </h3>
                <div className="space-y-2">
                  {tbdEvents.map((e) => (
                    <EventCard
        ...
        {course.consultation_hours && course.consultation_hours.length > 0 && (
          <section>
            <h3 className="text-xs font-semibold tracking-wider text-muted uppercase mb-2">
              {t('courseDetail.consultationHeading')}
            </h3>
            <div className="bg-card border border-border rounded-xl divide-y divide-border">
